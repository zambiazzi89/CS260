...
void HashTable::PrintAll() {
	...
	for (unsigned int i = 0; i < tableSize; i++) {
		...
		if (currentNode != nullptr && currentNode->key != UINT_MAX) {

			// For aesthetic reasons I separated the first element from the loop
			cout << "Key " << i << ": ";
			displayBid(currentNode->bid);
			currentNode = currentNode->next;

			// Subsequent elements have indentation
