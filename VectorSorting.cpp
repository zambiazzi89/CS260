...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	// Define pivot as the middle point
	int pivot = (begin + end) / 2;

	bool done = false;
	while (!done){

		// Increment low while it is less than pivot
		while (bids.at(low).title.compare(bids.at(pivot).title) < 0) {
			++low;
		}
		// Decrement high while it is greater than pivot
		while (bids.at(high).title.compare(bids.at(pivot).title) > 0) {
			--high;
		}

		// If low is greater than high, end loop, else swap bids
		if (low >= high) {
			done = true;
		} else {
...
void selectionSort(vector<Bid>& bids) {
	...
	for (unsigned int i = 0; i < bids.size(); ++i) {
	   ...
	   // Swap bids at i and indexSmallest
	   if (i != indexSmallest) {
